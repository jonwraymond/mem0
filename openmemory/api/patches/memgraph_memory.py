...
try:
    from langchain_memgraph.graphs.memgraph import MemgraphLangChain as Memgraph
except ImportError:
...
class MemoryGraph:
    ...
    def search(self, query, filters, limit=100):
        ...
        n_results = min(limit, len(search_outputs_sequence)) if limit else len(search_outputs_sequence)
        reranked_results = bm25.get_top_n(tokenized_query, search_outputs_sequence, n=n_results)

        search_results = []
        for item in reranked_results[:limit]:  # Apply limit to final results
